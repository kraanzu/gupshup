...
class HouseTree(CustomTree):
    def __init__(self) -> None:
        ...
        super().__init__(
            name, CustomNode(type="house_root", icon="")
        )
    ...
    def render_tree_label(
        ...
        if node.data.type == "house_root":
            ...
            color = (
                "green "
                if str(node.label) == self.selected[0]
                else "blue "
            )
        else:
            color = (
    ...
    async def expand_home(self) -> None:
        ...
        node = self.root.children[
            self.get_node_index(self.root, "HOME")
        ]
        await node.expand()
    ...
    def is_room_silent(self, house: str, room: str) -> bool:
        house_node = self.root.children[
            self.get_node_index(self.root, house)
        ]
        room_node = house_node.children[
            self.get_node_index(house_node, room)
        ]
        return room_node.data.silent

    async def add_house(self, name: str) -> None:
        await super().add_under_root(
            name, CustomNode(type="house", icon="ﳐ")
        )
    ...
    async def add_room(self, house: str, name: str) -> None:
        ...
        await self.add_under_child(
            house, name, CustomNode(type="room", icon="ﴘ")
        )
        self.refresh()
    ...
    def change_house_name(
        self, house: str, name: str
    ) -> None:
        super().change_name_parent(house, name)

    def change_house_data(
        self, house: str, param: str, value: str
    ) -> None:
        """
    ...
    def change_room_name(
        self, house: str, room: str, name: str
    ) -> None:
        super().change_name_child(house, room, name)

    def change_room_data(
        self, house: str, room: str, param: str, value: str
    ) -> None:
        """
    ...
    def increase_pending(
        ...
        current_pending = int(
            super().get_data_child(house, room, "pending")
        )
        super().change_data_child(
            house, room, "pending", str(current_pending + 1)
        )
        self.refresh()
