...
class Tui(App):
    ...
    async def on_load(self, _: events.Load) -> None:
        ...
        self.current_screen = (
            f"{self.current_house}/{self.current_room}"
        )

        self.member_lists: dict[
            str, MemberList
        ] = defaultdict(MemberList)
        self.member_scrolls: dict[
            str, ScrollView
        ] = defaultdict(ScrollView)
    ...
    async def perform_add_house(
        ...
        for name, rank in house.ranks.items():
            await self.member_lists[house.name].add_rank(
                name
            )
            self.member_lists[
                house.name
            ].change_data_parent(name, "color", rank.color)
            self.member_lists[
                house.name
            ].change_data_parent(name, "icon", rank.icon)

        for name, rank in house.member_rank.items():
            await self.member_lists[
                house.name
            ].add_user_to_rank(rank, name)
    ...
    async def perform_push_text(
        self, message: Message, local=False
    ) -> None:
        """
        ...
        if (
            not local
        ):  # check if local/offline data is not being pushed
            if self.current_screen == screen:
                ...
                if not self.house_tree.is_room_silent(
                    message.house, message.room
                ):
                    self.console.bell()

                self.house_tree.increase_pending(
                    message.house, message.room
                )

    async def perform_add_room(
        self, message: Message
    ) -> None:
        await self.house_tree.add_room(
            message.house, message.data["room"]
        )

    async def perform_clear_chat(
        self, message: Message
    ) -> None:
        screen = f"{message.house}/{message.room}"
    ...
    async def perform_del_room(
        self, message: Message
    ) -> None:
        screen = f"{message.house}/{message.room}"

        self.house_tree.del_room(
            message.house, message.room
        )
        self.chat_screen[screen].chats = ""
        if self.current_screen == screen:
            await self.update_chat_screen(
                message.house, "general"
            )

    async def perform_del_house(
        self, message: Message
    ) -> None:
        self.house_tree.del_house(message.house)
        await self.update_chat_screen("HOME", "general")

    async def perform_add_rank(
        self, message: Message
    ) -> None:
        await self.member_lists[message.house].add_rank(
            message.data["rank"]
        )

    async def perform_del_rank(
        self, message: Message
    ) -> None:
        await self.member_lists[message.house].del_rank(
            message.data["rank"]
        )

    async def perform_add_user_rank(
        self, message: Message
    ) -> None:
        await self.member_lists[
            message.house
    ...
    async def perform_del_user_rank(
        self, message: Message
    ) -> None:
        await self.member_lists[
            message.house
    ...
    async def perform_change_rank_color(
        self, message: Message
    ) -> None:
        await self.member_lists[
            message.house
    ...
    async def perform_change_rank_name(
        self, message: Message
    ) -> None:
        await self.member_lists[
            message.house
    ...
    async def perform_change_rank_icon(
        self, message: Message
    ) -> None:
        await self.member_lists[
            message.house
    ...
    async def perform_change_room_name(
        self, message: Message
    ) -> None:
        name = message.data["name"]
        self.house_tree.change_room_name(
            message.house, message.room, name
        )
        self.chat_screen[
            f"{message.house}/{name}"
        ] = self.chat_screen[
    ...
    async def perform_change_room_icon(
        self, message: Message
    ) -> None:
    ...
    async def perform_toggle_silent(
        self, message: Message
    ) -> None:
        self.house_tree.toggle_silent(
            message.house, message.room
        )
    ...
    async def on_mount(self, _: events.Mount) -> None:
        ...
        self.member_list_scroll = ScrollView(
    ...
    async def populate_local_data(self) -> None:
        ...
        for message in self.client.chats:
            if message.action == "push_text":
                await self.perform_push_text(
                    message, local=True
                )
            else:
                await eval(
                    f"self.perform_{message.action}(message)"
                )
    ...
    async def update_chat_screen(
        ...
        self.house_tree.change_room_data(
            house, room, "pending", "0"
        )
        self.banner.set_text(self.current_screen)
        self.house_tree.select(
            self.current_house, self.current_room
        )

        await self.refresh_screen()

    async def handle_tree_click(
        self, click: TreeClick
    ) -> None:
        """
